  ...
  return (
    ...
    >
      <Paper elevation={0} sx={{ width: '100%' }}>
        <Typography variant="h5" sx={{ marginBottom: 3, fontWeight: 700 }}>
          COMPTE RENDU
        </Typography>
        <Grid container spacing={2}>
          <Grid item xs={6}>
            <Typography>
              Numéro: <b>{taskData?.task?.number}</b>
            </Typography>
            <Typography>
              Date d'arrivée:{' '}
              <b>
                {taskData?.task?.startedAt
                  ? `${getFormatDateTime(taskData?.task?.startedAt)}`
            ...
            <Typography>
              Date de départ:{' '}
              <b>
                {taskData?.task?.finishedAt
                  ? `${getFormatDateTime(taskData?.task?.finishedAt)}`
                  : 'Pas encore finie'}
              </b>
            </Typography>
            <Typography>
              ...
              <br />
              <b>{taskData?.task?.address}</b>
            </Typography>
          </Grid>
          <Grid item xs={6}>
            <Typography sx={{ fontWeight: 700 }}>
              {taskData?.task?.client?.name}
            </Typography>
            <Typography>{taskData?.task?.client?.address}</Typography>
            {taskData?.task?.client?.email && (
              <Typography>{taskData?.task?.client?.email}</Typography>
            )}
            {taskData?.task?.client?.fix && (
              <Typography>{taskData?.task?.client?.fix}</Typography>
            )}
            {taskData?.task?.client?.mobile && (
              <Typography>{taskData?.task?.client?.mobile}</Typography>
            )}
          </Grid>
        </Grid>
          ...
          <Grid item xs={4}>
            ...
            {taskData?.task?.employeeSignature && (
              <>
                <Typography
                  sx={{ fontWeight: 700 }}
                >{`${taskData?.task?.employeeSignature?.creator?.firstName} ${taskData?.task?.employeeSignature?.creator?.lastName}`}</Typography>
                <img
                  src={taskData?.task?.employeeSignature?.base64Encoded}
                  width={100}
                  alt="Signature de l'intervenant"
          ...
          <Grid item xs={4}>
            ...
            {taskData?.task?.clientSignature && (
              <>
                <Typography sx={{ fontWeight: 700 }}>
                  {' '}
                  {taskData?.task?.clientSignature?.authorName}
                </Typography>
                <img
                  src={taskData?.task?.clientSignature?.base64Encoded}
                  width={100}
                  alt="Signature de l'intervenant"
                  title="Signature de l'intervenant"
                />
