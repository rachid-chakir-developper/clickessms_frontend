...
import { POST_FILE, PUT_FILE } from '../../../../_shared/graphql/mutations/MediaMutations';
...
export default function DialogAddFile({open, onClose, folderParent, type, fileToEdit}) {
    const  { setNotifyAlert,  setConfirmDialog} = useFeedBacks();
    const validationSchema = yup.object({
        name: yup .string('Entrez le nom de fichier').required(`Le nom est obligatoire`),
    });
    const formik = useFormik({
        initialValues: { fileUpload : null, nom : '', description : '', observation : '', folder : null },
    ...
    const handleOk = (fileForm) => {
        ...
        if(fileToEdit) updateFile({ variables: { id : fileToEdit.id, fileData, fileUpload } })
    ...
    React.useEffect(
        () => {
          if(open) {
            ...
            if(fileToEdit){
                formik.setValues({
                     ...formik.values, name: fileToEdit.name,
                     description: fileToEdit.description, observation: fileToEdit.observation, folder: folderParent.id
                    })
            }
          }
        },
                    ...
                    <DialogContent dividers>
                        <Grid container columns={{ xs: 4, sm: 8, md: 12 }}>
                            <Grid item xs={12}  sm={12} md={12}>
                                <Item>
                                    <TheFieldField variant="outlined" label="Séléctionner un fichier"
                            ...
                            <Grid item xs={12}  sm={12} md={12}>
                                <Item>
                                    <TheTextField variant="outlined" label="Nom de fichier" id="name"
