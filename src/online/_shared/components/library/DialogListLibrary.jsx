...
import { Add, ArrowBackIos, Delete, Edit, Folder, InsertDriveFile, InsertDriveFileOutlined, PlayCircleOutline } from '@mui/icons-material';
...
import { DELETE_FILE, DELETE_FOLDER } from '../../../../_shared/graphql/mutations/MediaMutations';
...
export default function DialogListLibrary({ dialogListLibrary, setDialogListLibrary }) {
  ...
  const [deleteFolder, { loading : loadingDeleteFolder }] = useMutation(DELETE_FOLDER, {
    onCompleted: (datas) => {
  ...
  const [deleteFile, { loading : loadingDeleteFile }] = useMutation(DELETE_FILE, {
      onCompleted: (datas) => {
  ...
  return (
    <>
      ...
      >
        <AppBar sx={{ position: 'relative' }}>
          <Toolbar>
            <IconButton
              edge="start"
              color="inherit"
              onClick={() => setDialogListLibrary({ ...dialogListLibrary, isOpen: false }) }
              aria-label="close"
            >
              <CloseIcon />
            </IconButton>
        ...
        {
          (!folder || (folder?.folders < 1 && folder?.files  < 1)) && !loadingFolder &&
          <Alert severity="warning" sx={{margin : 2 }}>Ce dossier est vide !</Alert>
        }
          ...
          return <div key={`folder${index}`}>
                  <ListItem button
                    ...
                    secondaryAction={
                      ...
                      <>
                        <IconButton edge="end" aria-label="delete" sx={{marginX : 0.5}}
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        <IconButton edge="end" aria-label="edit" sx={{marginX : 0.5}}
                          onClick={(e) => {
                            e.stopPropagation();
                    ...
                    <ListItemText primary={folderItem?.name} secondary={folderItem?.descreption} />
                  </ListItem>
                  <Divider />
                  </div>
              })
          ...
          {folder?.files?.map((file, index)=>{
            return <div key={`file${index}`}>
                    <ListItem button
                      ...
                      >
                      <ListItemIcon>
                        {file?.fileType !== 'IMAGE' && file?.fileType !== 'VIDEO' && <InsertDriveFileOutlined />}
                        {file?.fileType === 'IMAGE' && <Avatar sx={{ width: 24, height: 24 }} alt={file?.fileType?.name}  src={file?.file} />}
                        {file?.fileType === 'VIDEO' && <PlayCircleOutline />}
                      </ListItemIcon>
                      <ListItemText primary={file?.name} secondary={file?.descreption} />
                    </ListItem>
                    <Divider />
                    </div>
                })
      ...
      <DialogAddFolder open={openDialogFolder} onClose={closeDialogFolder} folderParent={folder} type={type} folderToEdit={folderToEdit}/>
      <DialogAddFile open={openDialogFile} onClose={closeDialogFile} folderParent={folder} type={type} fileToEdit={fileToEdit}/>
    </>
  );
}
