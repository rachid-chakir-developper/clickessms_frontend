...
export default function NotificationPanel() {
  ...
  const [markNotificationsAsSeen, { loading : loadingMarskAsSeen }] = useMutation(MARK_NOTIFICATIONS_AS_SEEN, {
      onCompleted: (datas) => {
  ...
  subscribeToMoreNotification({
    ...
    updateQuery: (prev, { subscriptionData }) => {
      ...
      const newNotificationItem = subscriptionData.data.onNotificationAdded.notification;
      if (prev.notifications.nodes.map(n => n.id).includes(newNotificationItem.id)) return prev;
      setNotifyAlert({
          isOpen: true,
  ...
  const loadMoreNotification = () => {
    if(!loadingNotifications && notificationsData?.notifications?.nodes && notificationsData?.notifications?.nodes.length >= 6 &&
      ...
      fetchMoreNotifications({
        ...
        updateQuery: (prev, { fetchMoreResult }) => {
          if (!fetchMoreResult || !fetchMoreResult?.notifications || !fetchMoreResult?.notifications?.nodes) return prev;
          setPage(page + 1)
          return {
            notifications: {
  ...
  const onMarkNotificationsAsSeen = () =>{
    if(notificationsData?.notifications?.nodes?.filter(n => !n.isSeen).length > 0){
      markNotificationsAsSeen({ variables : { ids: notificationsData?.notifications?.nodes?.filter(n => !n.isSeen).map(n => n?.id)}})
    }
  }
  React.useEffect(()=>{
    getNotifications()
