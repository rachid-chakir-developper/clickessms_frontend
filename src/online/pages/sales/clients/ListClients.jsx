...
import { DELETE_CLIENT, PUT_CLIENT_STATE } from '../../../../_shared/graphql/mutations/ClientMutations';
...
export default function ListClients() {
  ...
  const [getClients, { 
        ...
        }] = useLazyQuery(GET_CLIENTS, { variables: { clientFilter, page: paginator.page, limit: paginator.limit }})
      ...
      cache.modify({
        fields: {
          clients(existingClients = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedClients = existingClients.nodes.filter((client) =>
              readField('id', client) !== deletedClientId
            );
  ...
  const [updateClientState, { loading : loadingPutState }] = useMutation(PUT_CLIENT_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingClients && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {clientsData?.clients?.nodes.length < 1 && !loadingClients && <Alert severity="warning">Aucun client trouvé.</Alert>}
