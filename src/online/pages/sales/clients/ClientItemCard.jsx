...
export default function ClientItemCard({client, onDeleteClient, onUpdateClientState }) {
    ...
    <Card variant="outlined" sx={{ p: 1, display: 'flex', alignItems: 'center', gap: 2, }} >
      ...
      <Stack direction="column" spacing={2} alignItems="center">
        ...
        <Stack direction="row" alignItems="center" spacing={1.5}>
          ...
          <Tooltip title={!client?.isActive ? 'Activer' : 'Désactiver'}>
            <IconButton
              aria-label={!client?.isActive ? 'play' : 'pause'}
              sx={{ mx: 1 }}
              onClick={() => onUpdateClientState(client?.id)}
            >
              {!client?.isActive ? <PlayArrowRounded /> : <PauseRounded />}
            </IconButton>
          </Tooltip>
          ...
          {client?.folder && <Tooltip title="Pièces jointes">
            <IconButton aria-label="Attachment" size="small" sx={{ flexGrow: 0 }}
              onClick={()=> onOpenDialogListLibrary(client?.folder)}>
              <Folder fontSize="small" />
            </IconButton>
          </Tooltip>}
          <Tooltip title="Détails">
            <Link to={`/online/ventes/clients/details/${client?.id}`} className="no_style">
              <IconButton aria-label="edit" size="small">
