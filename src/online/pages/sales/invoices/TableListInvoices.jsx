...
import { Alert, MenuItem, Popover, Stack } from '@mui/material';
import AppLabel from '../../../../_shared/components/app/label/AppLabel';
import { Link, useNavigate } from 'react-router-dom';
import ProgressService from '../../../../_shared/services/feedbacks/ProgressService';
import TableFilterButton from '../../../_shared/components/table/TableFilterButton';
import { getFormatDate, getPaymentMethodLabel } from '../../../../_shared/tools/functions';
...
const headCells = [
    ...
    },
    {
        ...
        disableClickDetail: true,
        label: 'financeur',
        render: ({financier})=> <FinancierChip financier={financier} />
    },
    {
        id: 'clientTvaNumber',
        property: 'client_tva_number',
        exportField: 'client_tva_number',
        numeric: false,
        disablePadding: false,
    ...
    {
        id: 'emissionDate',
        property: 'emission_date',
        exportField: 'emission_date',
        numeric: false,
        disablePadding: false,
        isDefault: true,
        label: 'Date',
        render: ({emissionDate})=> getFormatDate(emissionDate)
    },
    {
        id: 'dueDate',
        property: 'validity_end_date',
    ...
    },
    {
        ...
        disableClickDetail: true,
        label: 'Status',
        render: (data)=> <InvoiceStatusLabelMenu invoice={data} />
    },
    {
        id: 'action',
        numeric: true,
...
function EnhancedTableToolbar(props) {
  const { selectedRows, numSelected, onFilterChange } = props;
...
export default function TableListInvoices({
  loading,
  rows,
  onDeleteInvoice,
  onFilterChange,
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }} >
        <EnhancedTableToolbar selectedRows={rows?.filter(i=> selected?.includes(i.id))} numSelected={selected.length} onFilterChange={(selectedColumns)=>setSelectedColumns(selectedColumns)}/>
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
                    ...
                    </StyledTableCell>
                    {
                      selectedColumns?.filter(c=>c?.id !== 'action')?.map((column, index) => {
                        return <StyledTableCell
                          ...
                          key={index}
                          onClick={()=> {if(!column?.disableClickDetail) navigate(`/online/ventes/factures/modifier/${row?.id}`)}}
                        >
