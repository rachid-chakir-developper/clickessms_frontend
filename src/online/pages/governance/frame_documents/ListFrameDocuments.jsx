...
export default function ListFrameDocuments() {
  ...
  const [frameDocumentFilter, setFrameDocumentFilter] = React.useState({ documentScope: 'GOVERNANCE'});
  const handleFilterChange = (newFilter) => {
    console.log('newFilter', newFilter);
    setFrameDocumentFilter({...newFilter, documentScope: 'GOVERNANCE'});
  };
  ...
  return (
    <Grid container spacing={2}>
      {canManageGovernanceModules && <Grid item xs={12}>
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', my: 3 }}>
          <Link
            to="/online/gouvernance/documents-trames/ajouter"
            className="no_style"
          >
