...
import { DELETE_THE_OBJECT, PUT_THE_OBJECT_STATE } from '../../../../_shared/graphql/mutations/TheObjectMutations';
...
export default function ListTheObjects() {
  ...
  const [getTheObjects, { 
        ...
        }] = useLazyQuery(GET_THE_OBJECTS, { variables: { theObjectFilter, page: paginator.page, limit: paginator.limit }})

  React.useEffect(() =>{
    getTheObjects()
  }, [theObjectFilter, paginator])

  const [deleteTheObject, { loading : loadingDelete }] = useMutation(DELETE_THE_OBJECT, {
    onCompleted: (datas) => {
      ...
      cache.modify({
        fields: {
          theObjects(existingTheObjects = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedTheObjects = existingTheObjects.nodes.filter((theObject) =>
  ...
  const [updateTheObjectState, { loading : loadingPutState }] = useMutation(PUT_THE_OBJECT_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingTheObjects && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {theObjectsData?.theObjects?.nodes?.length < 1 && !loadingTheObjects && <Alert severity="warning">Aucun objet trouvé.</Alert>}
