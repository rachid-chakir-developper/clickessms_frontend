...
import { POST_THE_OBJECT, PUT_THE_OBJECT } from '../../../../_shared/graphql/mutations/TheObjectMutations';
...
export default function AddTheObjectForm({ idTheObject, title}) {
    ...
    const validationSchema = yup.object({
        name: yup .string('Entrez le nom de objet').required('Le nom de objet est obligatoire')
      });
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            theObjectCopy.client = theObjectCopy.client ? theObjectCopy.client.id : null
    ...
    const [createTheObject, { loading : loadingPost }] = useMutation(POST_THE_OBJECT, {
        onCompleted: (data) => {
            console.log(data);
            setNotifyAlert({
                isOpen: true,
    ...
    const [updateTheObject, { loading : loadingPut }] = useMutation(PUT_THE_OBJECT, {
        onCompleted: (data) => {
            console.log(data);
            setNotifyAlert({
                isOpen: true,
        ...
        update(cache, { data: { updateTheObject } }) {
            ...
            cache.modify({
              fields: {
                theObjects(existingTheObjects = { totalCount: 0, nodes: [] }, { readField }) {
                    
                    const updatedTheObjects = existingTheObjects.nodes.map((theObject) =>
                        readField('id', theObject) === updatedTheObject.id ? updatedTheObject : theObject
                    );
    ...
    const [getTheObject, { loading : loadingTheObject }] = useLazyQuery(GET_THE_OBJECT, {
                    ...
                    <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                        ...
                        <Grid xs={12} sm={12} md={12}>
                            <Item sx={{ justifyContent: 'end', flexDirection : 'row' }}>
                                <Link to="/online/recuperations/objets/liste" className="no_style">
