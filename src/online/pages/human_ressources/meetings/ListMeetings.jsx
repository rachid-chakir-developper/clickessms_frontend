...
export default function ListMeetings() {
  ...
  const [meetingFilter, setMeetingFilter] = React.useState({meetingMode : 'INTERVIEW'});
  const handleFilterChange = (newFilter) => {
    console.log('newFilter', newFilter);
    setMeetingFilter({...newFilter, meetingMode : 'INTERVIEW'});
  };
    ...
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', my: 3 }}>
          <Link
            to="/online/ressources-humaines/cr-entretiens/ajouter"
            className="no_style"
          >
        ...
        <Box sx={{ flexGrow: 1 }}>
          ...
          >
            ...
            {meetingsData?.meetings?.nodes?.length < 1 && !loadingMeetings && (
              <Alert severity="warning">Aucun entretien trouvé.</Alert>
            )}
