...
import { DELETE_BENEFICIARY, PUT_BENEFICIARY_STATE } from '../../../../_shared/graphql/mutations/BeneficiaryMutations';
...
export default function ListBeneficiaries() {
  ...
  const [getBeneficiaries, { 
        ...
        } ]= useLazyQuery(GET_BENEFICIARIES, { variables: { beneficiaryFilter, page: paginator.page, limit: paginator.limit }})

  React.useEffect(() =>{
    getBeneficiaries()
  }, [beneficiaryFilter, paginator])

  const [deleteBeneficiary, { loading : loadingDelete }] = useMutation(DELETE_BENEFICIARY, {
    onCompleted: (datas) => {
      ...
      cache.modify({
        fields: {
          beneficiaries(existingBeneficiaries = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedBeneficiaries = existingBeneficiaries.nodes.filter((beneficiary) =>
  ...
  const [updateBeneficiaryState, { loading : loadingPutState }] = useMutation(PUT_BENEFICIARY_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingBeneficiaries && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {beneficiariesData?.beneficiaries?.nodes?.length < 1 && !loadingBeneficiaries && <Alert severity="warning">Aucun bénéficiaire trouvé.</Alert>}
              {beneficiariesData?.beneficiaries?.nodes?.map((beneficiary, index) => (
