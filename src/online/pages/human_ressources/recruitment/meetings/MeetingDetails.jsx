...
import JobCandidateChip from '../job_candidates/JobCandidate';
          ...
          <Grid item xs={5}>
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  <b>Date début: </b>{' '}
                  {`${getFormatDateTime(meetingData?.meeting?.startingDateTime)}`} <br />
                  <b>Date fin: </b>{' '}
                  {`${getFormatDateTime(meetingData?.meeting?.endingDateTime)}`}
                </Typography>
              </Paper>
          </Grid>
...
function MeetingOtherInfos({ meeting }) {
  return (
    ...
    >
      ...
      {meeting?.jobPosition && <Paper sx={{ padding: 1, marginY:1 }} variant="outlined">
        <Typography variant="h6" gutterBottom>
          Fiche besoin concernée
        </Typography>
        <JobPositionChip jobPosition={meeting?.jobPosition} />
      </Paper>}
      {meeting?.jobCandidate && <Paper sx={{ padding: 1, marginY:1 }} variant="outlined">
        <Typography variant="h6" gutterBottom>
          Candidat concerné
        </Typography>
        <JobCandidateChip jobCandidate={meeting?.jobCandidate} />
      </Paper>}
    </Paper>
  );
}
