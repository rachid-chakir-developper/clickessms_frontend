...
import { DELETE_EMPLOYEE_GROUP, PUT_EMPLOYEE_GROUP_STATE } from '../../../../../_shared/graphql/mutations/EmployeeGroupMutations';
...
export default function ListEmployeeGroups() {
  ...
  const [getEmployeeGroups, { 
        ...
        }] = useLazyQuery(GET_EMPLOYEE_GROUPS, { variables: { employeeGroupFilter, page: paginator.page, limit: paginator.limit }})

  React.useEffect(() =>{
    getEmployeeGroups()
  }, [employeeGroupFilter, paginator])

  const [deleteEmployeeGroup, { loading : loadingDelete }] = useMutation(DELETE_EMPLOYEE_GROUP, {
    onCompleted: (datas) => {
      ...
      cache.modify({
        fields: {
          employeeGroups(existingEmployeeGroups = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedEmployeeGroups = existingEmployeeGroups.nodes.filter((employeeGroup) =>
  ...
  const [updateEmployeeGroupState, { loading : loadingPutState }] = useMutation(PUT_EMPLOYEE_GROUP_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingEmployeeGroups && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {employeeGroupsData?.employeeGroups?.nodes?.length < 1 && !loadingEmployeeGroups && <Alert severity="warning">Aucun groupe de'employés trouvé.</Alert>}
              {employeeGroupsData?.employeeGroups?.nodes?.map((employeeGroup, index) => (
