...
export default function ListSuppliers() {
  ...
  const [
    ...
    variables: { supplierFilter, page: paginator.page, limit: paginator.limit },
  });
  ...
  const [deleteSupplier, { loading: loadingDelete }] = useMutation(
    DELETE_SUPPLIER,
    {
      onCompleted: (datas) => {
        ...
        cache.modify({
          fields: {
            suppliers(
              ...
              const updatedSuppliers = existingSuppliers.nodes.filter(
                (supplier) => readField('id', supplier) !== deletedSupplierId,
              );
  ...
  const [updateSupplierState, { loading: loadingPutState }] = useMutation(
    PUT_SUPPLIER_STATE,
    {
      onCompleted: (datas) => {
      ...
      <Grid item="true" xs={12}>
        <Box sx={{ flexGrow: 1 }}>
          ...
          >
            ...
            {suppliersData?.suppliers?.nodes.length < 1 &&
              !loadingSuppliers && (
