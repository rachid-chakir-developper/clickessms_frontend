...
import { Delete, PauseRounded, PlayArrowRounded, Edit } from '@mui/icons-material';
import { Link } from 'react-router-dom';

export default function UserItemCard({user, onDeleteUser, onUpdateUserState }) {
    ...
    <Card variant="outlined" sx={{ p: 1, display: 'flex', alignItems: 'center', gap: 2, }} >
      ...
      <Stack direction="column" spacing={2} alignItems="center">
        ...
        <Stack direction="row" alignItems="center" spacing={1.5}>
          { user?.employee && <Tooltip title={`L'employé: ${user?.employee?.firstName} ${user?.employee?.lastName}`}>
            <Link to={`/online/employes/details/${user?.employee?.id}`} className="no_style">
              <Avatar alt={`${user?.employee?.firstName} ${user?.employee?.lastName}`} 
          ...
          <Tooltip title="Modifier">
            <Link to={`/online/utilisateurs/modifier/${user?.id}`} className="no_style">
