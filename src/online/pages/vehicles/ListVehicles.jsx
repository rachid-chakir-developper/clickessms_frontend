...
import { DELETE_VEHICLE, PUT_VEHICLE_STATE } from '../../../_shared/graphql/mutations/VehicleMutations';
...
export default function ListVehicles() {
  ...
  const [getVehicles, { 
        ...
        }] = useLazyQuery(GET_VEHICLES, { variables: { vehicleFilter, page: paginator.page, limit: paginator.limit }})

  React.useEffect(() =>{
    getVehicles()
  }, [vehicleFilter, paginator])
  const [deleteVehicle, { loading : loadingDelete }] = useMutation(DELETE_VEHICLE, {
    onCompleted: (datas) => {
      ...
      cache.modify({
        fields: {
          vehicles(existingVehicles = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedVehicles = existingVehicles.nodes.filter((vehicle) =>
              readField('id', vehicle) !== deletedVehicleId
            );
  ...
  const [updateVehicleState, { loading : loadingPutState }] = useMutation(PUT_VEHICLE_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingVehicles && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {vehiclesData?.vehicles?.nodes?.length < 1 && !loadingVehicles && <Alert severity="warning">Aucun véhicule trouvé.</Alert>}
