...
import { DELETE_LETTER, PUT_LETTER_STATE } from '../../../../_shared/graphql/mutations/LetterMutations';
...
export default function ListLetters() {
  ...
  const [getLetters, { 
        ...
        }] = useLazyQuery(GET_LETTERS, { variables: { letterFilter, page: paginator.page, limit: paginator.limit }})

  React.useEffect(() =>{
    getLetters()
  }, [letterFilter, paginator])

  const [deleteLetter, { loading : loadingDelete }] = useMutation(DELETE_LETTER, {
    onCompleted: (datas) => {
      ...
      cache.modify({
        fields: {
          letters(existingLetters = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedLetters = existingLetters.nodes.filter((letter) =>
              readField('id', letter) !== deletedLetterId
            );
  ...
  const [updateLetterState, { loading : loadingPutState }] = useMutation(PUT_LETTER_STATE, {
    onCompleted: (datas) => {
    ...
    <Grid container spacing={2} >
        <Grid item="true" xs={12}>
            <Box sx={{display : 'flex', justifyContent : 'flex-end', my : 3}}>
              <Link to="/online/administratif/courriers/ajouter" className="no_style">
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingLetters && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {lettersData?.letters?.nodes?.length < 1 && !loadingLetters && <Alert severity="warning">Aucun courrier trouvé.</Alert>}
