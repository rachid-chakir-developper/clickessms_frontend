    ...
    <Card variant="outlined" sx={{ p: 1, display: 'flex', alignItems: 'center', gap: 2, }}>
      ...
      <Stack direction="column" spacing={2} alignItems="center">
        ...
        <Stack direction="row" alignItems="center" spacing={1.5}>
          ...
          <Tooltip title={!call?.isActive ? 'Activer' : 'Désactiver'}>
            <IconButton
              aria-label={!call?.isActive ? 'play' : 'pause'}
              sx={{ mx: 1 }}
              onClick={() => onUpdateCallState(call?.id)}
            >
              {!call?.isActive ? <PlayArrowRounded /> : <PauseRounded />}
            </IconButton>
          </Tooltip>
