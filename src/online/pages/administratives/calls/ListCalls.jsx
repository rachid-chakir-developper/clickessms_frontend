...
import { DELETE_CALL, PUT_CALL_STATE } from '../../../../_shared/graphql/mutations/CallMutations';
...
export default function ListCalls() {
  ...
  const [getCalls, { 
        ...
        }] = useLazyQuery(GET_CALLS, { variables: { callFilter, page: paginator.page, limit: paginator.limit }})
      ...
      cache.modify({
        fields: {
          calls(existingCalls = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedCalls = existingCalls.nodes.filter((call) =>
              readField('id', call) !== deletedCallId
            );
  ...
  const [updateCallState, { loading : loadingPutState }] = useMutation(PUT_CALL_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingCalls && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {callsData?.calls?.nodes?.length < 1 && !loadingCalls && <Alert severity="warning">Aucun appel trouvé.</Alert>}
