...
export default function EstablishmentServiceDetails() {
  ...
  const [getEstablishmentService, { loading : loadingEstablishmentService, data: establishmentServiceData }] = useLazyQuery(GET_ESTABLISHMENT_SERVICE)
React.useEffect(()=>{
  ...
  return (
    <Stack>
        {establishmentServiceData?.establishmentService && <EstablishmentServiceDetailsPage establishmentService={establishmentServiceData?.establishmentService} />}
    </Stack>
  );
}
    ...
    <Grid container spacing={3}>
        ...
        >
          ...
          >
            ...
            >
              ...
              {establishmentServiceType && establishmentServiceType!=='' && <Typography variant="body2">
                Type: 
                  {establishmentServiceType === 'PRIMARY' && <b> Primaire</b>}
                  {establishmentServiceType === 'SECONDARY' && <b> Sécondaire</b>}
              </Typography>}
            </Box>
          </Box>
        </Paper>
      ...
      <Grid item xs={12} sm={8}>
        <Paper sx={{ padding : 2}}>
          ...
          {establishment && <><Typography variant="h6" gutterBottom sx={{ mt:3 }}>
            Etablissement
          </Typography>
          <Paper sx={{ padding : 2}} variant="outlined">
            <Item>
          ...
          {establishmentServiceParent && <><Typography variant="h6" gutterBottom sx={{ mt:3 }}>
      ...
      {establishmentServiceChilds?.length > 0 && <Grid item xs={12} sm={12}>
        <Paper sx={{ padding : 2}}>
          <Typography variant="h6" gutterBottom>
            Les services fils
          </Typography>
          <Paper sx={{ padding : 2}} variant="outlined">
            <Grid container columns={{ xs: 4, sm: 8, md: 12 }}>
              {establishmentServiceChilds?.map((establishmentService, index) => (
