...
import {
  DELETE_ESTABLISHMENT_SERVICE,
  PUT_ESTABLISHMENT_SERVICE_STATE,
...
export default function ListEstablishmentServices() {
  ...
  const [establishmentServiceFilter, setEstablishmentServiceFilter] =
    React.useState(null);
  ...
  ] = useLazyQuery(GET_ESTABLISHMENT_SERVICES, {
    variables: {
      establishmentServiceFilter,
      page: paginator.page,
  ...
  const [deleteEstablishmentService, { loading: loadingDelete }] = useMutation(
    DELETE_ESTABLISHMENT_SERVICE,
    {
      onCompleted: (datas) => {
          ...
          fields: {
            establishmentServices(
              ...
              const updatedEstablishmentServices =
                existingEstablishmentServices.nodes.filter(
                  (establishmentService) =>
                    readField('id', establishmentService) !==
                    deletedEstablishmentServiceId,
                );

              console.log(
                'Updated establishmentServices:',
                updatedEstablishmentServices,
              );
  ...
  const [updateEstablishmentServiceState, { loading: loadingPutState }] =
    useMutation(PUT_ESTABLISHMENT_SERVICE_STATE, {
      onCompleted: (datas) => {
    ...
    <Grid container spacing={2}>
      ...
      <Grid item="true" xs={12}>
        <Box sx={{ flexGrow: 1 }}>
          ...
          >
            {loadingEstablishmentServices && (
              <Grid key={'pgrs'} item xs={2} sm={4} md={3}>
                <ProgressService type="mediaCard" />
              </Grid>
            )}
            {establishmentServicesData?.establishmentServices?.nodes.length <
              1 &&
              !loadingEstablishmentServices && (
                <Alert severity="warning">Aucun service trouvé.</Alert>
              )}
            {establishmentServicesData?.establishmentServices?.nodes?.map(
              (establishmentService, index) => (
                <Grid xs={2} sm={4} md={3} key={index}>
                  <Item>
                    <EstablishmentServiceItemCard
                      establishmentService={establishmentService}
                      onDeleteEstablishmentService={
                        onDeleteEstablishmentService
                      }
                      onUpdateEstablishmentServiceState={
                        onUpdateEstablishmentServiceState
                      }
                    />
                  </Item>
      ...
      <Grid item="true" xs={12}>
        <PaginationControlled
          totalItems={
            establishmentServicesData?.establishmentServices?.totalCount
