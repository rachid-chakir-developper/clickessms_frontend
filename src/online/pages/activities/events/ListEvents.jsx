...
import { DELETE_EVENT, PUT_EVENT_STATE } from '../../../../_shared/graphql/mutations/EventMutations';
...
export default function ListEvents() {
  ...
  const [getEvents, { 
        ...
        }] = useLazyQuery(GET_EVENTS, { variables: { eventFilter, page: paginator.page, limit: paginator.limit }})
      ...
      cache.modify({
        fields: {
          events(existingEvents = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedEvents = existingEvents.nodes.filter((event) =>
              readField('id', event) !== deletedEventId
            );
  ...
  const [updateEventState, { loading : loadingPutState }] = useMutation(PUT_EVENT_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingEvents && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {eventsData?.events?.nodes?.length < 1 && !loadingEvents && <Alert severity="warning">Aucun événement trouvé.</Alert>}
