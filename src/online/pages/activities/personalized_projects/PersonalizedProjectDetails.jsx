...
import { GET_RECAP_PERSONALIZED_PROJECT } from '../../../../_shared/graphql/queries/PersonalizedProjectQueries';
...
export default function PersonalizedProjectDetails() {
  let { idPersonalizedProject } = useParams();
  const [
    getPersonalizedProject,
    { loading: loadingPersonalizedProject, data: personalizedProjectData },
  ] = useLazyQuery(GET_RECAP_PERSONALIZED_PROJECT);
  ...
  return (
    <Stack>
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', my: 1 }}>
        <Link
          to={`/online/activites/projets-personnalises/modifier/${personalizedProjectData?.personalizedProject?.id}`}
          className="no_style"
        >
      ...
      {personalizedProjectData?.personalizedProject && (
        <PersonalizedProjectDetailsPage
          personalizedProject={personalizedProjectData?.personalizedProject}
        />
      )}
    </Stack>
  );
...
const PersonalizedProjectDetailsPage = ({ personalizedProject }) => {
  const {
    id,
    number,
    name,
