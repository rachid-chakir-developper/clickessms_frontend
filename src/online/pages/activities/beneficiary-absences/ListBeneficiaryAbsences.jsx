...
export default function ListBeneficiaryAbsences() {
  ...
  const [getBeneficiaryAbsences, { 
        ...
        }] = useLazyQuery(GET_BENEFICIARY_ABSENCES, { variables: { beneficiaryAbsenceFilter, page: paginator.page, limit: paginator.limit }})
      ...
      cache.modify({
        fields: {
          beneficiaryAbsences(existingBeneficiaryAbsences = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedBeneficiaryAbsences = existingBeneficiaryAbsences.nodes.filter((beneficiaryAbsence) =>
              readField('id', beneficiaryAbsence) !== deletedBeneficiaryAbsenceId
            );
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingBeneficiaryAbsences && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {beneficiaryAbsencesData?.beneficiaryAbsences?.nodes?.length < 1 && !loadingBeneficiaryAbsences && <Alert severity="warning">Aucune absence trouvé.</Alert>}
              {beneficiaryAbsencesData?.beneficiaryAbsences?.nodes?.map((beneficiaryAbsence, index) => (
