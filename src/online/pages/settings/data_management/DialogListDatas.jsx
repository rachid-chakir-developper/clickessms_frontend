...
export default function DialogListDatas({ open, onClose, data }) {
  ...
  const [getDatas, { loading: loadingDatas, data: datasData }] =
    useLazyQuery(GET_DATAS);
  React.useEffect(() => {
    if (open) {
      ...
      cache.modify({
        fields: {
          datas(existingDatas = [], { readField }) {
            const updatedDatas = existingDatas.filter(
              (data) => readField('id', data) !== deletedDataId,
            );
      ...
      >
        ...
        <DialogContent dividers>
          <List dense={true}>
            ...
            {datasData?.datas?.length < 1 && !loadingDatas && (
