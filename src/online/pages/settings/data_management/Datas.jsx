...
import { Grid, Box, Card, CardContent, CardMedia, IconButton, Stack, Typography, Tabs, Tab } from '@mui/material';
...
export default function Datas() {
  ...
  return (<>
                    ...
                    <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                      {moduleToManage.datas.map((data, index) => (
                        <Grid item xs={2} sm={4} md={4} key={index}>
                          <Item>
                            <Card sx={{height : '100%'}}>
                              <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent : 'space-around', height : '100%'}}>
                                <CardContent>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', p: 1, height: '100%' }}>
                                  <IconButton aria-label="previous" onClick={() => handleClickList({name : data.name, type : data.type})}>
                                    <List />
                                  </IconButton>
                                  <IconButton aria-label="play/pause" sx={{opacity : 0.4}} disabled>
                                    <Settings />
                                  </IconButton>
                                  <IconButton aria-label="next" onClick={() => handleClickAdd({name : data.name, type : data.type})}>
                                    <Add />
                                  </IconButton>
                                </Box>
                              </Box>
      ...
      <DialogListDatas open={openDialogList} onClose={closeDialogList} data={data}/>
    </>
  );
}