...
import { POST_DATA, PUT_DATA } from '../../../../_shared/graphql/mutations/DataMutations';
...
export default function DialogAddData({open, onClose, data, dataToEdit}) {
    ...
    const validationSchema = yup.object({
        name: yup .string('Entrez le nom de élément').required(`Le nom est obligatoire`),
    });
    ...
    const handleOk = (dataForm) => {
        if(dataToEdit) updateData({ variables: {
          id : dataToEdit.id,
          name : dataForm.name,
          typeData: data.type
        } })
        else createData({ variables: {
          name : dataForm.name,
          typeData: data.type
        } })
    };
    ...
    <div>
            ...
            >
                <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
                    {dataToEdit ? 'Modifer un élément' : 'Ajouter un élément'} "{data?.name}"
                </DialogTitle>
                <IconButton
                    aria-label="close"
                    onClick={onClose}
                    ...
                    <DialogContent dividers>
                        <Grid container columns={{ xs: 4, sm: 8, md: 12 }}>
                            <Grid item xs={12}  sm={12} md={12}>
                                <Item>
                                    <TheTextField variant="outlined" label="Nom de élément" id="name"
