...
import { DELETE_MATERIAL, PUT_MATERIAL_STATE } from '../../../_shared/graphql/mutations/MaterialMutations';
...
export default function ListMaterials() {
  ...
  const [getMaterials, { 
        ...
        }] = useLazyQuery(GET_MATERIALS, { variables: { materialFilter, page: paginator.page, limit: paginator.limit }})
      ...
      cache.modify({
        fields: {
          materials(existingMaterials = { totalCount: 0, nodes: [] }, { readField }) {
            ...
            const updatedMaterials = existingMaterials.nodes.filter((material) =>
              readField('id', material) !== deletedMaterialId
            );
  ...
  const [updateMaterialState, { loading : loadingPutState }] = useMutation(PUT_MATERIAL_STATE, {
    onCompleted: (datas) => {
        ...
        <Grid item="true" xs={12}>
          <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
              {loadingMaterials && <Grid key={'pgrs'}  item xs={2} sm={4} md={3}><ProgressService type="mediaCard" /></Grid>}
              {materialsData?.materials?.nodes?.length < 1 && !loadingMaterials && <Alert severity="warning">Aucun matériel trouvé.</Alert>}
