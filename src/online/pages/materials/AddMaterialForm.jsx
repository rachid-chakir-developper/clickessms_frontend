...
import { POST_MATERIAL, PUT_MATERIAL } from '../../../_shared/graphql/mutations/MaterialMutations';
...
export default function AddMaterialForm({ idMaterial, title}) {
    ...
    const validationSchema = yup.object({
        name: yup .string('Entrez le nom du matériel').required('Le nom du matériel est obligatoire'),
      });
    const formik = useFormik({
        initialValues: {
            image : undefined,  number : '', name : '', barCode : '',
            isBlocked : false, isStockAuto : true, designation : '',
            buyingPriceHt : 0, tva : 0, quantity : 0, description : '',
    ...
    const [createMaterial, { loading : loadingPost }] = useMutation(POST_MATERIAL, {
        onCompleted: (data) => {
            console.log(data);
            setNotifyAlert({
                isOpen: true,
              ...
              fields: {
                materials(existingMaterials = { totalCount: 0, nodes: [] }, { readField }) {
                    ...
                    const updatedMaterials = existingMaterials.nodes.map((material) =>
                        readField('id', material) === updatedMaterial.id ? updatedMaterial : material
                    );
    ...
    const [getMaterial, { loading : loadingMaterial }] = useLazyQuery(GET_MATERIAL, {
            ...
            { !loadingMaterial &&
                <form onSubmit={formik.handleSubmit}>
                    <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                        ...
                        <Grid xs={2} sm={4} md={4}>
                            <Item>
                                <TheTextField variant="outlined" label="Prix d'achat HT d'unité"
                                    InputProps={{
                                        endAdornment: <InputAdornment position="start">€</InputAdornment>,
                                    }}
                                    value={formik.values.buyingPriceHt}
                                    onChange={(e) => formik.setFieldValue('buyingPriceHt', e.target.value)}
                                    disabled={loadingPost || loadingPut}
                                />
                            </Item>
                            <Item>
                        ...
                        <Grid xs={2} sm={3} md={3}>
                            <Item>
                                <TheTextField variant="outlined" label="Code barre"
                        ...
                        <Grid xs={2} sm={2} md={2}>
                            <Item>
                                <TheSwitch variant="outlined" label="Stock auto"
                                    checked={formik.values.isStockAuto}
                                    value={formik.values.isStockAuto}
                                    onChange={(e) => formik.setFieldValue('isStockAuto', e.target.checked)}
                                    disabled={loadingPost || loadingPut}
                                />
                            </Item>
                        </Grid>
