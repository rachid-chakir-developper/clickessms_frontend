...
const AppMap = ({ tasks = [], users = [] }) => {
  ...
  const center =
    tasks.length > 0
      ? { lat: Number(tasks[0].latitude), lng: Number(tasks[0].longitude) }
              ...
              {users &&
                users.map((user, index) => (
                  <UserMarker
                    key={index}
                    position={{
                      lat: Number(user.currentLatitude),
                      lng: Number(user.currentLongitude),
                    }}
                    user={user}
                  />
                ))}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Avatar
            src={user?.employee ? user?.employee?.photo : user?.photo}
            alt={user?.username}
      ...
      >
        ...
        <Paper sx={{ position: 'absolute', zIndex: 1, width: '140px', p: 1 }}>
          ...
          >
            ...
            <b>Date début: </b>
            {`${getFormatDateTime(task?.startingDateTime)}`}
          </Typography>
        </Paper>
      </div>
    </OverlayView>
