...
import { GET_RECAP_THE_BACKUP } from '../../../../_shared/graphql/queries/TheBackupQueries';
...
export default function TheBackupDetails() {
  let { idTheBackup } = useParams();
  const [
    getTheBackup,
    { loading: loadingTheBackup, data: theBackupData },
  ] = useLazyQuery(GET_RECAP_THE_BACKUP);
  ...
  return (
    <Stack>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', my: 1 }}>
        <Link
          to="/online/informatique/sauvegardes/liste"
          className="no_style"
        >
        ...
        <Link
          to={`/online/informatique/sauvegardes/modifier/${theBackupData?.theBackup?.id}`}
          className="no_style"
        >
      ...
      {theBackupData?.theBackup && (
        <TheBackupDetailsPage
          theBackup={theBackupData?.theBackup}
        />
      )}
    </Stack>
  );
...
const TheBackupDetailsPage = ({ theBackup }) => {
  const {
    id,
    number,
    name,
