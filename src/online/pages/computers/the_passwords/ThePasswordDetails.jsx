...
export default function ThePasswordDetails() {
  let { idThePassword } = useParams();
  const [
    getThePassword,
    { loading: loadingThePassword, data: thePasswordData },
  ] = useLazyQuery(GET_RECAP_THE_PASSWORD);
  ...
  return (
    <Stack>
      ...
      {thePasswordData?.thePassword && (
        <ThePasswordDetailsPage
          thePassword={thePasswordData?.thePassword}
        />
      )}
    </Stack>
  );
...
const ThePasswordDetailsPage = ({ thePassword }) => {
  const {
    id,
    number,
    name,
