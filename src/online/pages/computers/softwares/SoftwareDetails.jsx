...
import { GET_RECAP_SOFTWARE } from '../../../../_shared/graphql/queries/SoftwareQueries';
...
export default function SoftwareDetails() {
  ...
  const [
    getSoftware,
    { loading: loadingSoftware, data: softwareData },
  ] = useLazyQuery(GET_RECAP_SOFTWARE);
  ...
  return (
    <Stack>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', my: 1 }}>
        <Link
          to="/online/informatique/logiciels/liste"
          className="no_style"
        >
        ...
        <Link
          to={`/online/informatique/logiciels/modifier/${softwareData?.software?.id}`}
          className="no_style"
        >
...
const SoftwareDetailsPage = ({ software }) => {
  const {
    id,
    number,
    name,
