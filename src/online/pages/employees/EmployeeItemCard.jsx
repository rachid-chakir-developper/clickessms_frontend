  ...
  return (
    <Card variant="outlined" sx={{ p: 1, display: 'flex', alignItems: 'center', gap: 2, }} >
      <Tooltip title={`${employee?.firstName} ${employee?.lastName}`}>
        <CardMedia component="img" width="100" height="100" alt={`${employee?.firstName} ${employee?.lastName}`}
          src={ employee?.photo ? employee?.photo : "https://mui.com/static/images/cards/real-estate.png"}
      ...
      <Stack direction="column" spacing={2} alignItems="center">
        ...
        <Stack direction="row" alignItems="center" spacing={1.5}>
          ...
          <Tooltip title={!employee?.isActive ? 'Activer' : 'Désactiver'}>
            <IconButton
              aria-label={!employee?.isActive ? 'play' : 'pause'}
              sx={{ mx: 1 }}
              onClick={() => onUpdateEmployeeState(employee?.id)}
            >
              {!employee?.isActive ? <PlayArrowRounded /> : <PauseRounded />}
            </IconButton>
          </Tooltip>
          ...
          {employee?.folder && <Tooltip title="Pièces jointes">
            <IconButton aria-label="Attachment" size="small" sx={{ flexGrow: 0 }}
              onClick={()=> onOpenDialogListLibrary(employee?.folder)}>
