...
export default function AddEmployeeForm({ idEmployee, title}) {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            birthDate : dayjs(new Date()), position : '', hiringDate : dayjs(new Date()) ,
            probationEndDate : dayjs(new Date()) , workEndDate : dayjs(new Date()) , startingSalary : 0,
              ...
              fields: {
                employees(existingEmployees = { totalCount: 0, nodes: [] }, { readField }) {
                    ...
                    const updatedEmployees = existingEmployees.nodes.map((employee) =>
                        readField('id', employee) === updatedEmployee.id ? updatedEmployee : employee
                    );
    ...
    const [getEmployee, { loading : loadingEmployee }] = useLazyQuery(GET_EMPLOYEE, {
                ...
                <form onSubmit={formik.handleSubmit}>
                    <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                        ...
                        <Grid xs={2} sm={4} md={4}>
                            ...
                            <Item>
                                <TheTextField variant="outlined" label="Position"
                        ...
                        <Grid xs={2} sm={4} md={4}>
                            <Item>
                                <TheTextField variant="outlined" label="Salaire de départ"
                                    InputProps={{
                                        endAdornment: <InputAdornment position="start">€</InputAdornment>,
                                    }}
                                    value={formik.values.startingSalary}
                                    onChange={(e) => formik.setFieldValue('startingSalary', e.target.value)}
                                    disabled={loadingPost || loadingPut}
                                />
                            </Item>
                            <Item>
                            ...
                            <Item>
                                <TheDesktopDatePicker
                                    ...
                                    onChange={(date) => formik.setFieldValue('probationEndDate', date)}
                                    disabled={loadingPost || loadingPut}
                                />
                            </Item>
                            <Item>
