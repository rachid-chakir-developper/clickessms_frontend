...
import { GET_RECAP_BOX_IDEA } from '../../../../_shared/graphql/queries/BoxIdeaQueries';
...
export default function BoxIdeaDetails() {
  let { idBoxIdea } = useParams();
  const [
    getBoxIdea,
    { loading: loadingBoxIdea, data: boxIdeaData },
  ] = useLazyQuery(GET_RECAP_BOX_IDEA);
  ...
  return (
    <Stack>
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', my: 1 }}>
        <Link
          to={`/online/qualites/boite-idees/modifier/${boxIdeaData?.boxIdea?.id}`}
          className="no_style"
        >
...
const BoxIdeaDetailsPage = ({ boxIdea }) => {
  const {
    id,
    number,
    name,
