...
import { ArrowRight, Close, SkipNextRounded, SkipPreviousRounded } from '@mui/icons-material';
const MediaModal = ({ open, handleClose, images = [], videos = [], currentIndex = 0 }) => {
  ...
  return (
    ...
    >
      <Fade in={open}>
        <Stack sx={{ height : '100%', alignItems : 'center', justifyContent : 'center', padding: 20}}>
            <Box sx={{ display : 'flex', alignItems : 'flex-start', justifyContent : 'center'}}>
                <Box sx={{ alignItems : 'center', justifyContent : 'center'}}>
                    {(images && images?.length > 0) && <img src={images[currentMediaIndex]?.image} alt="Media" style={{ maxWidth: '100%', maxHeight: '80vh' }} />}
            ...
            sx={{position : 'absolute', top : 20, right: 20, padding : 2, zIndex: 1000, backgroundColor: "#e1e1e1"}} 
             aria-label="next" onClick={handleClose} >
                <Close />
            </IconButton>
        </Stack>
      </Fade>
