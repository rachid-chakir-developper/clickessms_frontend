...
export default function DialogSelectLocationMap({open, onClose, localisation}) {
    ...
    const [center, setCenter] = React.useState({lat: 49.4279197, lng: 1.067133});
    React.useEffect(
      () => {
        if(open) {
          ...
          setTimeout(()=>{
            if(localisation.lat != "" && localisation.lng != ""){
              const latlng = {lat : Number(localisation.lat), lng : Number(localisation.lng)}
        ...
        if (status === 'OK' && results[1]) {
          ...
          addressComponents.forEach((component) => {
            component.types.forEach((type) => {
              ...
              if (type === 'postal_code') updatedAddress.zipCode = component.long_name;
            });
          });
          ...
          setLocation({ latitude: latLng.lat, longitude: latLng.lng, address: null })
        }
      });
    };
  ...
  placeService.getDetails({ placeId }, (result, status) => {
    if (status === 'OK' && result.geometry && result.geometry.location) {
      ...
      if(updatedAddress.latitude && updatedAddress.longitude){
        const latlng = {lat : Number(updatedAddress.latitude), lng : Number(updatedAddress.longitude)}
        setCenter(latlng);
        let markerAux = {...marker}
        markerAux.lat = latlng.lat
        markerAux.lng = latlng.lng
